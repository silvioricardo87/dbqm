...
# Both name directories on the user's disk, not text on a screen: they
# stay Portuguese in every language, or an export lands beside the ones
# already there instead of with them.
_GROUP_CATEGORY = "grupos"
...
def export_query_csv(result: QueryResult, table: str = "", params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_QUERY_CATEGORY, label, result.connection_name, params, "csv")
...
def export_query_json(result: QueryResult, table: str = "", params: dict | None = None) -> str:
    ...
    label = table or result.query_name
    filepath = _build_filepath(_QUERY_CATEGORY, label, result.connection_name, params, "json")
    data = {
...
def export_query_txt(result: QueryResult, table: str = "", params: dict | None = None) -> str:
    ...
    label = table or result.query_name
    filepath = _build_filepath(_QUERY_CATEGORY, label, result.connection_name, params, "txt")
...
def export_individual_txt(
    ...
    filepath = _build_filepath(_QUERY_CATEGORY, result.query_name, result.connection_name, params, "txt")

    lines = []

    # Parameters
    if params:
        lines.append(t("export.label_params") + ":")
...
def _build_query_txt_lines(result: QueryResult) -> list[str]:
    ...
    lines = []
    lines.append(f'{t("export.label_query")}: {result.query_name}')
    lines.append(f'{t("export.label_connection")}: {result.connection_name}')
    lines.append(f'{t("export.label_rows")}: {result.row_count}')
    lines.append(f'{t("export.label_time")}: {result.elapsed:.2f}s')
    lines.append("")
...
def _status_label(status: str) -> str:
    ausente = "x " + t("export.status_absent_short")
    return {"OK": "v OK", "OK*": "v OK*", "DIFF": "! DIFF",
            "ABSENT": ausente}.get(status, status)
...
def export_group_csv(group_result: GroupResult, params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_GROUP_CATEGORY, group_result.group_name, params=params, ext="csv")
    ...
    with filepath.open("w", newline="", encoding="utf-8") as f:
        ...
        header = ([t("export.column_key"), t("export.column_query")]
                  + compare_columns + [t("export.column_status")])
        ...
        for key in all_keys:
            ...
            result_row = (["", t("export.label_result")]
                          + [_status_label(s) for s in col_statuses]
                          + [_status_label(overall)])
...
def export_group_json(group_result: GroupResult, params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_GROUP_CATEGORY, group_result.group_name, params=params, ext="json")
...
def export_group_txt(group_result: GroupResult, params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_GROUP_CATEGORY, group_result.group_name, params=params, ext="txt")
...
def _flat_status_label(status: str) -> str:
    return {"OK": t("export.flat_equal"), "OK*": t("export.flat_equal_norm"),
            "DIFF": t("export.flat_different"),
            "ABSENT": t("export.flat_absent")}.get(status, status)
...
def export_group_flat_csv(group_result: GroupResult, params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_GROUP_CATEGORY, group_result.group_name, params=params, ext="csv", extra="_flat")
    ...
    with filepath.open("w", newline="", encoding="utf-8") as f:
        ...
        writer = csv.writer(f)
        for comp in group_result.comparisons:
            f.write(f'# {t("export.label_column")}: {comp.column}\n')
            header = ([t("export.column_key")] + query_names
                      + [t("export.column_status")])
            writer.writerow(header)
            for row in comp.rows:
                cells = [row.key_value]
                for qn in query_names:
...
def export_group_flat_json(group_result: GroupResult, params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_GROUP_CATEGORY, group_result.group_name, params=params, ext="json", extra="_flat")
...
def export_group_flat_txt(group_result: GroupResult, params: dict | None = None) -> str:
    ...
    filepath = _build_filepath(_GROUP_CATEGORY, group_result.group_name, params=params, ext="txt", extra="_flat")
    ...
    for comp in group_result.comparisons:
        ...
        for row in comp.rows:
            parts = [str(row.key_value).ljust(col_w["key"])]
            for qn in query_names:
                val = row.values.get(qn)
        ...
        # Summary
        lines.append(f'  {t("comparison.equal")} '
                     f"{comp.equal_count}/{comp.total_keys}")
        if comp.normalized_count > 0:
            lines.append(f'  {t("comparison.normalized")} '
                         f"{comp.normalized_count}/{comp.total_keys}")
        lines.append(f'  {t("comparison.different")} '
                     f"{comp.diff_count}/{comp.total_keys}")
        lines.append(f'  {t("comparison.absent")} '
                     f"{comp.absent_count}/{comp.total_keys}")
        lines.append("")
