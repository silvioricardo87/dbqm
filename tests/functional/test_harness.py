...
def test_the_seed_is_what_the_documents_describe(local_db, capsys):
    """Three customers, four orders, one view -- the numbers every QA
    scenario in docs/qa/ counts on. Change the seed, change the documents."""
    assert run_cli(["sql", "SELECT COUNT(*) AS n FROM customers", "local", "-f", "json"]) is True
    assert json.loads(capsys.readouterr().out)["data"]["rows"] == [[3]]
    assert run_cli(["sql", "SELECT COUNT(*) AS n FROM orders", "local", "-f", "json"]) is True
