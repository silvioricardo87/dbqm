...
def test_an_update_is_refused_and_nothing_changes(read_only_db, capsys):
    code, body = envelope(["sql", "UPDATE customers SET status='X' WHERE id=1", "ro", "-f", "json"], capsys)
    assert code == 2
...
def test_force_write_lifts_the_guard_for_one_call(read_only_db, capsys):
    code, body = envelope(
        ["sql", "UPDATE customers SET status='X' WHERE id=1", "ro", "--force-write", "--commit", "-f", "json"],
        capsys,
    )
    assert code == 0
    ...
    # never saved
    code, body = envelope(["sql", "UPDATE customers SET status='A' WHERE id=1", "ro", "--commit", "-f", "json"], capsys)
    assert code == 2 and body["error"]["code"] == "read_only"


# QA-RO-003
def test_a_select_passes(read_only_db, capsys):
    code, body = envelope(["sql", "SELECT name FROM customers WHERE id=1", "ro", "-f", "json"], capsys)
    assert code == 0
...
def test_explain_passes(read_only_db, capsys):
    code, body = envelope(["sql", "SELECT id FROM customers", "ro", "--explain", "-f", "json"], capsys)
    assert code == 0
...
def test_a_hand_written_explain_query_plan_passes(read_only_db, capsys):
    ...
    code, body = envelope(["sql", "EXPLAIN QUERY PLAN SELECT id FROM customers", "ro", "-f", "json"], capsys)
    assert code == 0
...
def test_two_statements_are_refused_by_count(read_only_db, capsys):
    code, body = envelope(["sql", "SELECT 1; DROP TABLE customers", "ro", "-f", "json"], capsys)
    assert code == 2
    assert body["error"]["code"] == "read_only"
    assert "more than one statement" in body["error"]["message"]
    assert "customers" in _tables(capsys)


# QA-RO-007
@pytest.mark.parametrize("sql", [
    "INSERT INTO customers VALUES (4, 'Dora', 'A')",
...
def test_every_write_verb_is_refused(read_only_db, capsys, sql):
    ...
    assert body["error"]["code"] == "read_only"
    assert body["error"]["message"] == REFUSAL
    code, count = envelope(["sql", "SELECT COUNT(*) FROM customers", "local", "-f", "json"], capsys)
    assert count["data"]["rows"] == [[3]]
    assert sorted(_tables(capsys)) == ["customers", "orders"]
...
def test_force_write_on_a_writable_connection_changes_nothing(local_db, capsys):
    code, body = envelope(
        ["sql", "UPDATE customers SET status='X' WHERE id=1", "local", "--force-write", "--commit", "-f", "json"],
        capsys,
    )
    assert code == 0
    assert body["data"]["rows_affected"] == 1
...
def test_run_of_a_writing_query_is_refused(read_only_db, capsys):
    ...
    code, _ = envelope(
        ["query", "add", "atualiza", "--connection", "local",
         "--sql", "UPDATE customers SET status='X' WHERE id=1", "-f", "json"],
        capsys,
    )
    assert code == 0
