...
| QA-CALL-011 | Given a routine that prints `DBMS_OUTPUT.PUT_LINE('RETURN=I am not the return value')` itself and returns 7 / When called / Then `data.return_value == "7"` and the printed line stays in `warnings`, untouched · unit: `tests/core/test_object_browser.py::TestExecuteRoutineHandsValuesBack::test_a_routine_printing_RETURN_no_longer_shadows_the_real_one` | manual | oracle | — |
