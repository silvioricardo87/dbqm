...
| QA-EXPORT-011 | Given a saved query with a declared default and `run ... -e json` / When the exported file is read / Then the file name carries the default (`minimum-12`) and the rows are the ones the default actually filtered | functional | all | tests/functional/test_export.py::test_run_export_carries_the_query_own_default |
