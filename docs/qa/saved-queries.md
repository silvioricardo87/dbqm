...
|---|---|---|---|---|
| QA-QUERY-001 | Given `dbqm query add ativos --connection local --sql "SELECT id, name FROM customers WHERE status = 'A' ORDER BY id"` / When `dbqm run ativos -f json` / Then exit 0, `command == "run"`, `data.query_name == "ativos"`, `data.rows == [[1,"Ana"],[3,"Caio"]]` | functional | all | tests/functional/test_run.py::test_query_add_then_run_end_to_end |
...
| QA-QUERY-008 | Given `-e csv` / When `dbqm run ativos -e csv -f json` / Then `data.exported` is an existing `.csv` whose header is `id,name`, and `data.format == "csv"` | functional | all | tests/functional/test_run.py::test_export_writes_the_file_and_reports_it |
