...
| QA-QUERY-012 | Dado `-p naoexiste=1` numa consulta que nao declara esse parametro / Quando `dbqm run ativos -p naoexiste=1 -f json` / Entao exit 2, `validation`, `Consulta 'ativos' nao declara o parametro 'naoexiste'.` — antes rodava sem filtrar e devolvia as linhas como resultado | functional | all | tests/functional/test_run.py::test_a_param_the_query_does_not_declare_is_refused |
