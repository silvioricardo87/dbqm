...
| QA-GROUP-003 | Dado o grupo `pedidos` / Quando `dbqm run-group pedidos` (table) / Entao exit 5 e `DIVERGENTE` no stdout; para `clientes`, exit 0 e `CONSISTENTE` | functional | all | tests/functional/test_run_group.py::test_table_format_prints_the_verdict_with_the_same_exit |
...
| QA-GROUP-005 | Dado `--flat -e html` / Quando `dbqm run-group pedidos --flat -e html -f json` / Entao exit 2, `usage`, mensagem `--flat nao tem versao HTML. ...` e nada foi executado: o historico nao ganha registro | functional | all | tests/functional/test_run_group.py::test_flat_with_html_is_refused_before_anything_runs |
...
| QA-GROUP-007 | Dado um grupo que nao existe / Quando `dbqm run-group nope -f json` / Entao exit 2, `not_found`, `Grupo 'nope' nao encontrado.` | functional | all | tests/functional/test_run_group.py::test_an_unknown_group_is_not_found |
| QA-GROUP-008 | Dado `group add` com uma so consulta / Quando `dbqm group add um --query cli_local --join-key id -f json` / Entao exit 2, `validation`, `Selecione pelo menos 2 consultas.` | functional | all | tests/functional/test_run_group.py::test_a_group_needs_two_queries |
...
| QA-GROUP-011 | Dado um grupo cuja chave `id` vem de `cliente_id` (quatro pedidos, dois clientes) / Quando `dbqm run-group por_cliente -f json` / Entao exit 5, `warnings` traz uma linha por lado (`Chave 'id' tem valores repetidos em 'pc_local': 2 linha(s) fora da comparacao.`) e `comparisons[0].duplicate_rows == {"pc_local": 2, "pc_local2": 2}` | functional | all | tests/functional/test_run_group.py::test_a_repeated_join_key_is_reported_not_swallowed |
...
| QA-GROUP-014 | Dado a tela **Comparacao** da TUI e o grupo `pedidos` (divergente) / Quando executado / Entao a barra traz `pedidos`, `2 consultas` e `DIVERGENTE`, e o widget carrega a comparacao com `diff_count == 1` | functional | all | tests/ui/test_functional_screens.py::test_group_run_runs_a_group_and_shows_the_verdict |
...
| QA-GROUP-018 | Dado o mesmo grupo / Quando executado / Entao o primeiro `warnings` e `Grupo 'sem_colunas' nao define colunas para comparar; comparando as comuns: valor.` | functional | all | tests/functional/test_run_group.py::test_deriving_the_columns_is_said_out_loud |
| QA-GROUP-019 | Dado um grupo sem colunas cujas consultas so tem a chave em comum / Quando executado / Entao exit 2, `validation`, `Grupo 'so_id' nao define colunas para comparar e as consultas nao tem nenhuma coluna comum alem de 'id'.` | functional | all | tests/functional/test_run_group.py::test_nothing_common_beyond_the_key_is_refused |
| QA-GROUP-020 | Dado `group add --query q --query q` / Quando executado / Entao exit 2, `validation`, `Consulta "ped_local" repetida. Um grupo compara consultas distintas.` — o indice e por nome, entao a consulta se compararia consigo mesma | functional | all | tests/functional/test_run_group.py::test_a_group_cannot_name_the_same_query_twice |
| QA-GROUP-010 | Dado um grupo cuja consulta le uma tabela inexistente / Quando `dbqm run-group quebrado -f json` / Entao exit 4, `sql_error`, mensagem `Erro na consulta 'quebrada': no such table: nao_existe` | functional | all | tests/functional/test_run_group.py::test_a_failing_query_names_itself |
