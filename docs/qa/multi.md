...
| QA-MULTI-005 | Dado `-c local -c local` / Quando executado / Entao exit 2, `usage`, `Conexao 'local' repetida. Give at least two distinct connections with -c/--connection.` | functional | all | tests/functional/test_multi.py::test_the_same_connection_twice_is_refused |
