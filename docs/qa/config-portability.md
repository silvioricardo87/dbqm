...
| QA-PORT-006 | Given a path that does not exist / When `import-config missing.dbqm --password x -f json` / Then exit 2, `not_found`, `File "missing.dbqm" not found.` | functional | all | tests/functional/test_portability.py::test_a_missing_bundle_is_not_found |
