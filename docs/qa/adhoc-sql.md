...
| QA-SQL-017 | Dado `-e json` num DDL / Quando `dbqm sql "CREATE TABLE auditoria (id INTEGER)" local -e json -f json` / Entao exit 2, `usage`, `... DDL does not return any.` e `objects` nao lista `auditoria` | functional | all | tests/functional/test_sql.py::test_export_on_a_ddl_is_refused_and_nothing_is_created |
...
| QA-SQL-023 | Dado um caminho terminado em `.sql` que nao existe / Quando `dbqm sql caminho.sql local -f json` / Entao exit 2, `not_found`, `File "<path>" not found.` — antes era lido como SQL e voltava `Unsupported SQL type` | functional | all | tests/functional/test_sql.py::test_a_missing_sql_file_says_so |
