...
| QA-SQL-022 | Dado `-p naoexiste=1` num SQL que nao usa esse bind / Quando executado / Entao exit 2, `validation`, `O SQL nao usa o parametro 'naoexiste'.` | functional | all | tests/functional/test_sql.py::test_a_param_the_statement_never_binds_is_refused |
| QA-SQL-023 | Dado um caminho terminado em `.sql` que nao existe / Quando `dbqm sql caminho.sql local -f json` / Entao exit 2, `not_found`, `Arquivo '<caminho>' nao encontrado.` — antes era lido como SQL e voltava `Tipo de SQL nao suportado` | functional | all | tests/functional/test_sql.py::test_a_missing_sql_file_says_so |
