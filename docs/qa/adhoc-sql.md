...
counts on is `tests/functional/conftest.py::SEED`: `customers` (3 rows: Ana/A,
Bia/I, Caio/A), `orders` (4 rows), a unique index `ix_customers_name`, a view
`v_active`. Envelope shape and exit codes: [output-contract.md](output-contract.md).
Read-only refusals: [read-only-guard.md](read-only-guard.md).

| ID | Scenario | Layer | Engine | Test |
|---|---|---|---|---|
| QA-SQL-001 | Given the `local` connection / When `dbqm sql "SELECT id, name FROM customers ORDER BY id" local -f json` / Then exit 0, `data.rows` are the 3 seeded rows, `data.columns == ["id","name"]` and `data.sql_type == "SELECT"` | functional | all | tests/functional/test_sql.py::test_select_returns_the_seed_rows |
| QA-SQL-002 | Given `-p id=2` / When `dbqm sql "SELECT name FROM customers WHERE id = :id" local -p id=2 -f json` / Then exit 0 and `data.rows == [["Bia"]]` — the bind reached the driver | functional | all | tests/functional/test_sql.py::test_a_param_reaches_the_statement |
| QA-SQL-003 | Given an UPDATE without `--commit` / When `dbqm sql "UPDATE customers SET status='X' WHERE id=1" local -f json` / Then exit 2, `error.code == "usage"`, the message `DML requires --commit to confirm the operation.` and the row still reads `A` on a following read | functional | all | tests/functional/test_sql.py::test_dml_without_commit_is_refused_before_running |
| QA-SQL-004 | Given the same UPDATE with `--commit` / When it runs and then `dbqm sql "SELECT status FROM customers WHERE id=1" local -f json` on a second call / Then the first returns `rows_affected == 1` and `committed == true`, the second reads `X` | functional | all | tests/functional/test_sql.py::test_dml_with_commit_persists_for_the_next_call |
...
| QA-SQL-006 | Given a DDL the driver rejects (`CREATE TABLE customers (id INTEGER)`, it already exists) / When run with `-f json` / Then exit 4, `error.code == "sql_error"` and the driver's message (`table customers already exists`) | functional | all | tests/functional/test_sql.py::test_a_ddl_the_driver_rejects_is_sql_error |
| QA-SQL-007 | Given `--explain` / When `dbqm sql "SELECT id FROM customers" local --explain -f json` / Then exit 0 and `data.plan` is a non-empty list of strings mentioning `customers` | functional | all | tests/functional/test_sql.py::test_explain_returns_a_plan |
...
| QA-SQL-010 | Given a file `consulta.sql` holding `SELECT COUNT(*) AS n FROM orders` / When `dbqm sql <path> local -f json` / Then the file is read and `data.rows == [[4]]` | functional | all | tests/functional/test_sql.py::test_a_sql_file_path_is_read |
| QA-SQL-011 | Given `-e csv`, `-e json`, `-e txt`, `-e html` / When `dbqm sql "SELECT id, name FROM customers" local -e <fmt> -f json` / Then exit 0, `data.exported` points at an existing file with the format's extension (`.htm` for html), `data.format == <fmt>`, and the content carries `Ana` | functional | all | tests/functional/test_sql.py::test_export_writes_a_file_per_format |
...
| QA-SQL-014 | Given `-f table` (the default) / When `dbqm sql "SELECT name FROM customers WHERE id=1" local` / Then exit 0 and `Ana` appears on stdout | functional | all | tests/functional/test_sql.py::test_table_format_prints_the_rows |
| QA-SQL-016 | Given `-e csv` on an UPDATE / When `dbqm sql "UPDATE customers SET status='X' WHERE id=1" local --commit -e csv -f json` / Then exit 2, `usage`, `--export needs a command that returns rows; UPDATE does not return any.`, the row still reads `A` and no file was written — the refusal comes before it runs | functional | all | tests/functional/test_sql.py::test_export_on_a_dml_is_refused_before_the_write |
...
| QA-SQL-019 | Given `--explain -e csv` / When `dbqm sql "SELECT id FROM customers" local --explain -e csv -f json` / Then exit 0 and the file carries the `plan` header and the plan — a plan is a result set, and this branch returns before the guard | functional | all | tests/functional/test_sql.py::test_explain_exports_the_plan |
