...
| QA-SQL-010 | Given a file `query.sql` holding `SELECT COUNT(*) AS n FROM orders` / When `dbqm sql <path> local -f json` / Then the file is read and `data.rows == [[4]]` | functional | all | tests/functional/test_sql.py::test_a_sql_file_path_is_read |
