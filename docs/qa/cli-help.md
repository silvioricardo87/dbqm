...
| QA-HELP-007 | Given `dbqm --help` at 80 columns / Then no line is wider than 80 characters | unit | all | tests/cli/test_help_shape.py::test_no_line_is_wider_than_eighty_columns |
...
| QA-HELP-012 | Given stdin is not a terminal / When `dbqm` runs with no arguments / Then exit 2, stdout empty, and stderr carries the "needs a terminal" message plus the grouped help (`run-group`, `describe-cli`) | unit | all | tests/cli/test_tui_command.py::test_the_bare_invocation_refuses_a_pipe_with_the_help |
| QA-HELP-019 | Given stdin is not a terminal / When `dbqm` (main entry point) runs with no arguments / Then `SystemExit` carries code 2 | unit | all | tests/test_cli.py::TestMainEntryPoint::test_main_no_args_refuses_without_a_terminal |
