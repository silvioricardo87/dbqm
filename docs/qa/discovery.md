...
|---|---|---|---|---|
| QA-DISC-001 | Given `local` / When `dbqm objects local -f json` (default type TABLE) / Then exit 0, `data.obj_type == "TABLE"` and `data.objects == ["clientes","pedidos"]` | functional | all | tests/functional/test_discovery.py::test_objects_lists_the_seed_tables |
| QA-DISC-002 | Given `local` / When `dbqm objects local --type VIEW -f json` / Then `data.objects == ["v_ativos"]` | functional | all | tests/functional/test_discovery.py::test_objects_lists_the_seed_view |
...
| QA-DISC-005 | Given `clientes` / When `dbqm describe clientes local -f json` / Then `data.object_type == "TABLE"`, columns `id` (`is_pk == true`, `INTEGER`), `nome` (`nullable == false`, `TEXT`), `status`; and `data.indexes == [{"name":"ix_clientes_nome","columns":["nome"],"is_unique":true}]` | functional | all | tests/functional/test_discovery.py::test_describe_reports_pk_nullability_and_the_unique_index |
| QA-DISC-006 | Given `pedidos` / When `dbqm describe pedidos local -f json` / Then the `cliente_id` column carries `fk_ref == "clientes.id"` | functional | all | tests/functional/test_discovery.py::test_describe_reports_the_foreign_key |
| QA-DISC-007 | Given `v_ativos` / When `dbqm describe v_ativos local -f json` / Then `data.object_type == "VIEW"`, columns `id` and `nome`, and `data.sql_definition` starts with `CREATE VIEW v_ativos` | functional | all | tests/functional/test_discovery.py::test_describe_a_view_brings_its_definition |
...
| QA-DISC-009 | Given `-f table` / When `dbqm describe clientes local` / Then exit 0 and `PK`, `ix_clientes_nome` and `UNIQUE` appear on stdout | functional | all | tests/functional/test_discovery.py::test_describe_table_format_prints_keys_and_indexes |
| QA-DISC-010 | Given `clientes` / When `dbqm rows clientes local -f json` / Then `data.rows` are the 3 seeded rows, `row_count == 3`, `total_count == 3`, `limit == 100`, `offset == 0` | functional | all | tests/functional/test_discovery.py::test_rows_returns_the_seed |
...
| QA-DISC-014 | Given `-f csv` / When `dbqm rows clientes local -f csv` / Then stdout starts with `id,nome,status` and has one line per row | functional | all | tests/functional/test_discovery.py::test_rows_csv_prints_a_header_and_the_rows |
