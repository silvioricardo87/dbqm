...
| QA-OUT-008 | Given the `ro` connection / When `dbqm sql "DELETE FROM customers" ro -f json` / Then exit 2 and `error.code == "read_only"` | functional | all | tests/functional/test_output_contract.py::test_exit_2_read_only |
...
| QA-OUT-014 | Given `local` and `local2` disagreeing on one order / When `dbqm run-group orders -f json` / Then exit 5 with `ok == true` on stdout (the comparison finished; the answer is "no") and `data.all_match == false` | functional | all | tests/functional/test_run_group.py::test_a_real_divergence_exits_5 |
