...
| QA-TPL-002 | Dado `t1` / Quando `template add t1 --content x -f json` / Entao exit 2, `validation`, `Template "t1" ja existe.` e o `content` original continua | functional | all | tests/functional/test_template.py::test_add_refuses_a_duplicate_and_keeps_the_original |
...
| QA-TPL-007 | Dado stdin que nao e terminal / Quando `template rm t1 -f json` sem `--yes` / Entao exit 2, `usage`, `Use --yes para remover sem confirmacao.` e `t1` continua | functional | all | tests/functional/test_template.py::test_rm_without_yes_off_a_tty_is_refused |
