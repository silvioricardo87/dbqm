...
|---|---|---|---|---|
| QA-RO-001 | Given `ro` / When `dbqm sql "UPDATE clientes SET status='X' WHERE id=1" ro -f json` / Then exit 2, `error.code == "read_only"`, the message `Connection 'ro' is read-only. Use --force-write to send it anyway.` and the row still reads `A` | functional | all | tests/functional/test_read_only.py::test_an_update_is_refused_and_nothing_changes |
...
| QA-RO-003 | Given `ro` / When `dbqm sql "SELECT nome FROM clientes WHERE id=1" ro -f json` / Then exit 0 and `data.rows == [["Ana"]]` | functional | all | tests/functional/test_read_only.py::test_a_select_passes |
| QA-RO-004 | Given `ro` / When `dbqm sql "SELECT id FROM clientes" ro --explain -f json` / Then exit 0 and `data.plan` is not empty | functional | all | tests/functional/test_read_only.py::test_explain_passes |
| QA-RO-005 | Given `ro` / When `dbqm sql "EXPLAIN QUERY PLAN SELECT id FROM clientes" ro -f json` (SQLite's native EXPLAIN, written by hand) / Then exit 0 — the guard recognises `QUERY PLAN` as an EXPLAIN that only reads | functional | sqlite | tests/functional/test_read_only.py::test_a_hand_written_explain_query_plan_passes |
| QA-RO-006 | Given `ro` / When `dbqm sql "SELECT 1; DROP TABLE clientes" ro -f json` / Then exit 2, `read_only`, a message carrying `more than one statement`, and `clientes` still exists | functional | all | tests/functional/test_read_only.py::test_two_statements_are_refused_by_count |
