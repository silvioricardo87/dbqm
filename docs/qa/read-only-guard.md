...
| QA-RO-010 | Dado `ro` e uma consulta salva cujo SQL e um UPDATE / Quando `dbqm run atualiza -c ro -f json` / Entao exit 2, `error.code == "usage"`, mensagem `Only SELECT statements are allowed.` e a linha continua `A` — `run` nao tem caminho de escrita em conexao nenhuma, e por isso nao tem `--force-write` | functional | all | tests/functional/test_read_only.py::test_run_of_a_writing_query_is_refused |
