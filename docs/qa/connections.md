...
| QA-CONN-019 | Given `g1` with a description / When `group update g1 --folder pasta -f json` then `show` / Then `folder == "pasta"` and `description`, `queries`, `join_key` untouched | functional | all | tests/functional/test_connection.py::test_group_update_changes_only_what_it_is_given |
